...
zparseopts -E -D -F - -help=opt_help h=opt_help -gst=opt_gst -rpicam=opt_rpicam \
    ...
    -gstprot:=opt_gstprot -streamcam:=opt_streamcam -stillcam:=opt_stillcam
...
Options:
  ...
  --gstprot <value>     Set stream protocol rtp (RTP) or mpeg (MPEG-TS) (default: rtp)
  --streamcam <0|1>     Camera index for streaming (default: 0)
  --stillcam <0|1>      Camera index for still images (default: 0)
  -h, --help            Show this help message and exit

Note:
  - If GStreamer (--gst) is used, --gstver and --gstprot can be specified; otherwise, they are ignored.

Example:
  $0 --rpicam --width 1280 --height 720 --framerate 25
  $0 --gst --clientip 192.168.0.100 --clientport 6000
  $0 --gst --gstprot mpeg --clientip 192.168.0.120 --clientport 5000
...
[[ -n "$opt_gstprot" ]] && GST_PROT="${opt_gstprot[2]}"
[[ -n "$opt_streamcam" ]] && STREAMCAM="${opt_streamcam[2]}"
[[ -n "$opt_stillcam" ]] && STILLCAM="${opt_stillcam[2]}"

typeset -A CAMERA_PATH_MAP
typeset -A CAMERA_NAME_MAP

while IFS= read -r line; do
    if [[ $line =~ '^([0-9]+) : ([^ ]+) \[.*\] \(([^)]*)\)' ]]; then
        CAMERA_PATH_MAP[${match[1]}]=${match[3]}
        CAMERA_NAME_MAP[${match[1]}]=${match[2]}
    fi
done <<< "$(rpicam-hello --list-cameras 2>/dev/null)"
...
function start_preview() {
    ...
    if [[ "$STREAMER" == "rpicam-vid" ]]; then
        rpicam-vid --camera $STREAMCAM --width $WIDTH --height $HEIGHT --framerate $FRAMERATE \
        ...
        PREVIEW_PID=$!
    elif [[ "$STREAMER" == "gst" && "$GST_PROT" == "mpeg" ]]; then
        CAMERA_PATH=${CAMERA_PATH_MAP[$STREAMCAM]}
        "${GST_CMD}" libcamerasrc camera-name="$CAMERA_PATH" af-mode=continuous ! \
            capsfilter caps=video/x-raw,width=$WIDTH,height=$HEIGHT,format=NV12,interlace-mode=progressive,framerate=${FRAMERATE}/1 ! \
            x264enc speed-preset=1 threads=1 ! \
...
start_preview
echo "Stream running... (Press 'c' = capture, 'r' = restarting preview, 's' = stop and exit)"

while true; do
    read -sk 1 key
    case $key in
        c)
            CAMERA_NAME=${CAMERA_NAME_MAP[$STILLCAM]}
            [[ $STILLCAM == $STREAMCAM ]] && stop_preview
            echo "Capturing image..."
            TIMESTAMP=$(date +%Y%m%d_%H%M%S)
            if [[ "$CAMERA_NAME" == "ov64a40" ]]; then
                rpicam-still --camera $STILLCAM --autofocus-window 0.33,0.33,0.33,0.33 --hdr single-exp --autofocus-mode auto --autofocus-on-capture --denoise cdn_hq --vflip --hflip --gain 1.0 --saturation 1.0 -t 1000 --raw --output "$CAPTURE_DIR/image_${TIMESTAMP}.jpg" -n
                echo "Image saved: $CAPTURE_DIR/image_${TIMESTAMP}.jpg"
                rpicam-still --camera $STILLCAM --autofocus-window 0.33,0.33,0.33,0.33 --hdr single-exp --autofocus-mode auto --autofocus-on-capture --denoise cdn_hq --vflip --hflip --gain 1.0 --saturation 0.0 -t 1000 --output "$CAPTURE_DIR/image_bw_${TIMESTAMP}.jpg" -n
                echo "Image saved: $CAPTURE_DIR/image_bw_${TIMESTAMP}.jpg"
            else
                rpicam-still --camera $STILLCAM --raw --output "$CAPTURE_DIR/image_${TIMESTAMP}.jpg" --hdr single-exp --timeout 1000 -n
                echo "Image saved: $CAPTURE_DIR/image_${TIMESTAMP}.jpg"
                rpicam-still --camera $STILLCAM --output "$CAPTURE_DIR/image_bw_${TIMESTAMP}.jpg" --saturation 0.0 --hdr single-exp --timeout 1000 -n
                echo "Image saved: $CAPTURE_DIR/image_bw_${TIMESTAMP}.jpg"
            fi
            [[ $STILLCAM == $STREAMCAM ]] && { echo "Restarting preview..."; start_preview; }
            ;;
        r)
            stop_preview
            echo "Restarting preview..."; start_preview;
            ;;
        s)
            stop_preview
            echo "Exiting script."
